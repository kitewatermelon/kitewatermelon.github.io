...
- Data Augmentation module, $\mathnormal{Aug(·)}$: 입력 샘플 $\mathcal{x}$에 대하여 random augmentation을 통해 $\tilde{x} = \mathcal{Aug(x)}$를 얻는자. 이는 데이터에 대한 2개의 다른 뷰를 의미한다.
- Encoder Network, $\mathnormal{Enc(·)}$: $\mathnormal{D_E}$ = 2048   
$\tilde{x}$를 $\mathcal{R}^\mathnormal{D_E}$ unit hypersphere의 벡터 $\mathnormal{r}$로 매핑한다.
$$
\begin{equation}
r = Enc(x) \in R^{D_E} \\
\end{equation}
$$
- Projection Network, $\mathnormal{Proj(·)}$: $\mathnormal{D_P}$ = 128    
$\mathnormal{r}$을 $\mathnormal{D_p}$차원의 MLP나 single linear layer를 이용하여 $\mathnormal{z}$로 만든다. 이를 다시 normalize 하여 unit hypersphere에 놓이도록 한다.
$$
\begin{equation}
z = Proj(r) \in R^{D_P}
\end{equation}
$$
Supervised Contrastive Loss를 이해하기 위해 Self-Supervised Contrastive Loss를 알아야 하며 Notaion을 알 필요가 있다.
batch size를 N이라고 하자, 한 mini batch에는 N개의 랜덤으로 샘플링된 sample, label set이 있을 것이다. 
> original mini batch는 $\mathnormal{\{x_k, y_k\}_{l=1...N}}$로 표현할 수 있으며    
> augmentation을 적용한 mini batch는 2N개의 데이터가 있을 것이다. $\mathnormal{\{\tilde{x_l}, \tilde{y_l}\}_{l=1...2N}}$로 표현할 수 있을 것이다. $\mathnormal{\tilde{x}_{2k}}$와 $\mathnormal{\tilde{x}_{2k-1}}$은 같은 sample의 서로 다른 view이며 label은 $\mathnormal{\tilde{y}_{2k}}$=$\mathnormal{\tilde{y}_{2k-1}}$=$\mathnormal{y_k}$이다. 

### Self-Supervised Contrastive Losses
multiviewed batch 안에 $\mathnormal{i \in I = {1...2N}}$은  augmented sample의 index set이라고 하고 $\mathnormal{j(i)}$는 같은 sample의 다른 view라고 할 때 Self-Supervised Contrastive Loss는 다음과 같이 표현할 수 있다. 
$$
\begin{equation}
\mathcal{L}^{\text{self}}
= \sum_{i \in \mathcal{I}} \mathcal{L}^{\text{self}}_i
= -\sum_{i \in \mathcal{I}}
\log
\frac{
...
\end{equation}
$$
$\mathnormal{z_l}$=$\mathnormal{Proj(Enc(\tilde{x}))} \in \mathcal{R^{D_P}}$이고 $\mathnormal{\cdot}$은 내적을 의미한다. $\mathnormal{\tau} \in R^+$는 scaler temperture parameter다. 마지막으로 $A(i) \equiv{I /\{i\}}$ index $\mathnormal{i}$는 anchor로 부르며, index $\mathnormal{j(i)}$는 positive, 나머지 2(N-1)개를 negatives라고 정의한다.

### Supervised Contrastive Losses
기존의 Contrastive Loss에서 레이블이 정보를 추가해줘야 하기 때문에 아래의 3, 4번 수식 처럼 변경을 해야 한다. 
$\mathnormal{P(i)} \equiv{p \in A(i): \tilde{y}_p = \tilde{y}_i}$는 multiviewd batch에서 모든 positives의 index set이며 $\mathnormal{|P(i)|}$는 cardinalty(집합의 원소 개수)이다.

$$
\begin{equation}
\mathcal{L}^{\text{sup}}_{\text{out}}
= \sum_{i \in \mathcal{I}} \mathcal{L}^{\text{sup}}_{\text{out}, i}
= \sum_{i \in \mathcal{I}}
\frac{-1}{|P(i)|}
\sum_{p \in P(i)}
\log
...
= \sum_{i \in \mathcal{I}} \mathcal{L}^{\text{sup}}_{\text{in}, i}
= \sum_{i \in \mathcal{I}}
- \log
\left(
\frac{1}{|P(i)|}
...
두 Loss가 겉보기에는 같아보일지라도 Log 함수가 concave function(오목한 함수)이기 때문에 Jensen's inequality로 인하여 이론상 $\mathcal{L}^{sup}_{in} \leq{\mathcal{L}^{sup}_{out}}$이며 실제로도 ImageNet top-1 accuracy에서 각각 67.4%와 78.7%로 $\mathcal{L}^{sup}_{out}$가 더 성능이 좋았다. (이 부분에 대한 수학적 유도를 본문 3.2.2 후반부와 Supplementary 6장에서 제공하나, 본 post에서는 다루지 않겠다.)
...
인코더는 보통 자주 쓰이는 resnet-50,101,200($\mathnormal{D_E}= 2048$) 세가지를 통해 비교했으며 Augmentation 전략으로는 AutoAugment, RandAugment, SimAugment, Stacked RandAugment 네가지를 사용했으며 resnet-50에서 SupCon과 CE 모두에서 AutoAugment 전략이 가장 성능이 좋았다. Stacked RandAugment는 resnet-200에서 성능이 제일 좋았다. (자세한 성능은 아래의 table 참고)
...
$\tau$=0.1로 잡은 이유는 작은 temperture가 주는 이점이 있기 때문이다. 그럼에도 너무 작은 temperture는 gradient가 불안정하다(fig 4 참고).
